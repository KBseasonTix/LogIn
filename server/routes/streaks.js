...
router.get('/user/:userId/history', async (req, res) => {
  try {
    ...
    // Get recent history
    const history = streakTracker.streakHistory.slice(-days).map(entry => ({
      date: entry.date,
...
router.get('/community/:communityId', async (req, res) => {
  try {
    ...
    const communityStreaks = users
      .map(user => {
        const streakData = streakTrackers.find(st => st.userId.toString() === user._id.toString());
